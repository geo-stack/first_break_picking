...
from first_break_picking.data.data_utils import sinc_interp_factor, plotseis

'''
This file is mostly coded by Aleksei Tarasov
...
class SeisModel:
    ...
    def get_random_model(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Generate a random shot and its fist arrivals

        Returns
        -------
        model_output: ndarray
            Seismic shot
        picking_output: ndarray
            picks
        """
    ...
    def _prepare_path(self) -> List[Path]:
        path = os.path.abspath(os.path.join(os.path.abspath(__file__), "../signals/"))
        path = Path(path)
